...
sw_count_violations <- function(gage_all){
  ...
  f11 <- c("f05","f10","f20", "f30","f40","f50","f60","f70",
          "f80","f90","f95")
  
  f9 <- c("f05","f20", "f30","f40","f50","f60","f70","f80","f95")
  
  f_all <- list(f27,f21,f15,f11,f9)
  ...
  for (i in 1:length(f_all)) {
    ...
    epochs <- 250
    cv_results <- sw_k_foldcv(build_model,k=5,epochs=epochs,batch_size=294,Y=Y2,X=X,data=d)
